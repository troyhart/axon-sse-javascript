...
import com.example.axonssejavascript.values.ErrorMessage;
...
public interface ControllerAdviceSupport {
  ...
  // TODO: keep an eye out for still needing this. I don't think we do however.
  //  @ExceptionHandler(value = AxonServerRemoteQueryHandlingException.class)
  //  default ResponseEntity<ErrorMessage> handleAxonServerRemoteQueryHandlingException(
  //      AxonServerRemoteQueryHandlingException ex, WebRequest request) {
  //    HttpStatus status = ex.getErrorCode().equals("AXONIQ-5001") ? HttpStatus.NOT_FOUND : HttpStatus.BAD_REQUEST;
  //    String token = UUID.randomUUID().toString();
  //
  //    logger().error(tagMessage(token, "QUERY FAILURE; REQUEST: " + request), ex);
  //    logger().warn(tagMessage(token, String.format("ERROR CODE: %s; SERVER: %s", ex.getErrorCode(), ex.getServer())));
  //    ErrorMessage body = new ErrorMessage.Builder().code(status.name()).message(token).detail(
  //        new ErrorMessage.Builder().code("exception")
  //            .message(ex.getExceptionDescriptions().stream().collect(Collectors.joining("\n * ", "\n * ", ""))).build())
  //        .build();
